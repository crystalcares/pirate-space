...
type TopTrader = Tables<'top_traders'>;
...
export default function FeaturedExchangersManager() {
    const { user } = useAuth();
    ...
    const fetchTradersAndProfiles = async () => {
        ...
        if (tradersRes.error) toast.error('Failed to fetch top clients.');
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        ...
        const toastId = toast.loading(editingTrader ? 'Updating client...' : 'Creating client...');
        ...
        try {
            ...
            const dataToSubmit: Omit<TablesInsert<'top_traders'>, 'id' | 'created_at'> = {
            ...
            const { error } = editingTrader
                ? await supabase.from('top_traders').update(dataToSubmit).eq('id', editingTrader.id)
                : await supabase.from('top_traders').insert(dataToSubmit);
            
            if (error) throw error;

            toast.success('Client saved successfully!', { id: toastId });
    ...
    return (
        <Card className="bg-card/70 border-border/50 backdrop-blur-xl">
            <CardHeader className="flex flex-row items-center justify-between">
                <div>
                    <CardTitle>Manage Top Clients</CardTitle>
                    <CardDescription>Manually add, edit, or remove users from the curated "Top Clients" list.</CardDescription>
                </div>
                <Dialog open={isDialogOpen} onOpenChange={(open) => { setIsDialogOpen(open); if (!open) setEditingTrader(null); }}>
                    ...
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>{editingTrader ? 'Edit' : 'Add'} Client</DialogTitle>
                        </DialogHeader>
                        <Form {...form}>
