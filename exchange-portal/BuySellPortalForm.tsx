...
export default function BuySellPortalForm({ setCurrentStep }: { setCurrentStep: (step: number) => void }) {
    ...
    const [payAmount, setPayAmount] = useState<number | ''>(initialAmount ? parseFloat(initialAmount) : (mode === 'buy' ? 100 : 1));
    ...
    const toOptions = useMemo(() => Array.from(new Set(activePairs.filter(p => p.from === fromCurrency).map(p => p.to))), [activePairs, fromCurrency]);
    ...
    useEffect(() => {
        ...
        const currentActivePairs = mode === 'buy' ? buyPairs : sellPairs;
        if (currentActivePairs.length > 0) {
            const firstPair = currentActivePairs[0];
            newParams.set('from', firstPair.from);
            newParams.set('to', firstPair.to);
        }
        setPayAmount(mode === 'buy' ? 100 : 1);
        setSearchParams(newParams, { replace: true });
    }, [mode, buyPairs, sellPairs, setSearchParams]);

    useEffect(() => {
        if (!fromCurrency || activePairs.length === 0) return;
        const availableTo = activePairs.filter(p => p.from === fromCurrency).map(p => p.to);
        if (!availableTo.includes(toCurrency)) {
            const newParams = new URLSearchParams(searchParams);
            newParams.set('to', availableTo[0] || '');
            setSearchParams(newParams);
        }
    }, [fromCurrency, toCurrency, activePairs, searchParams, setSearchParams]);

    const handleFromCurrencyChange = (newFrom: string) => {
        const newParams = new URLSearchParams(searchParams);
        newParams.set('from', newFrom);
        const newToOptions = activePairs.filter(p => p.from === newFrom).map(p => p.to);
    ...
    const { finalAmount, fiatValue, pair } = useMemo(() => {
        if (payAmount === '' || payAmount <= 0) return { finalAmount: 0, fiatValue: 0, pair: null };
        ...
        const calculatedFinalAmount = payAmount * finalRate;
        const calculatedFiatValue = payAmount * fiatRate;
        
        return { finalAmount: calculatedFinalAmount, fiatValue: calculatedFiatValue, pair: foundPair };
    }, [payAmount, fromCurrency, toCurrency, getConversionRate, exchangePairs]);
    ...
    const handleCreateExchange = async () => {
        ...
        if (error) {
            ...
            sendExchangeCreationWebhook({
                ...
                exchangeType: mode === 'buy' ? 'Buy Crypto' : 'Sell Crypto',
    ...
    return (
        <Card className="shadow-2xl bg-card/90 backdrop-blur-sm border-border/20">
            <CardContent className="p-6 space-y-4">
                ...
                {/* You Send */}
                <div>
                    <Label className="text-sm text-muted-foreground">{mode === 'buy' ? 'You pay' : 'You sell'}</Label>
                    <div className="flex items-center justify-between bg-muted/50 rounded-lg p-2 mt-1">
                        ...
                        <Select value={fromCurrency} onValueChange={handleFromCurrencyChange}>
                            ...
                            <SelectContent>
                                {fromOptions.map(c => <SelectItem key={c} value={c}><CurrencySelectItem currency={c} /></SelectItem>)}
                            </SelectContent>
                        </Select>
                    </div>
                    {mode === 'sell' && <p className="text-xs text-muted-foreground mt-1 text-right">≈ $<AnimatedNumericText value={fiatValue} precision={2} /></p>}
                </div>
                ...
                <div>
                    ...
                    <div className="flex items-center justify-between bg-muted/50 rounded-lg p-2 mt-1">
                        ...
                        <Select value={toCurrency} onValueChange={handleToCurrencyChange}>
                            ...
                            <SelectContent>
                                {toOptions.map(c => <SelectItem key={c} value={c}><CurrencySelectItem currency={c} /></SelectItem>)}
                            </SelectContent>
                        </Select>
                    </div>
                </div>
